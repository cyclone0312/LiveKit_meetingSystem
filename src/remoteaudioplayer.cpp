...
{
    // 连接信号槽，确保音频数据在主线程处理
    connect(this, &RemoteAudioPlayer::audioDataReady,
            this, &RemoteAudioPlayer::onAudioDataReady,
            Qt::QueuedConnection);
...
bool RemoteAudioPlayer::initAudioOutput(int sampleRate, int channels)
{
    ...
    // 设置较大的缓冲区以避免断续
    m_audioSink->setBufferSize(sampleRate * channels * sizeof(int16_t) / 5); // 200ms 缓冲
...
void RemoteAudioPlayer::onAudioDataReady(QByteArray data, int sampleRate, int channels)
{
    // 此槽在主线程执行
    if (!m_running.load())
    {
