...
class MediaCapture : public QObject {
  ...
  Q_PROPERTY(bool microphoneActive READ isMicrophoneActive NOTIFY
                 microphoneActiveChanged)
  Q_PROPERTY(QVideoSink *videoSink READ videoSink WRITE setVideoSink NOTIFY
                 videoSinkChanged)
  Q_PROPERTY(QStringList availableCameras READ availableCameras NOTIFY
                 availableCamerasChanged)
  Q_PROPERTY(QStringList availableMicrophones READ availableMicrophones NOTIFY
                 availableMicrophonesChanged)
  Q_PROPERTY(int currentCameraIndex READ currentCameraIndex WRITE
...
public:
  ...
  // 【关键】完全重置 LiveKit 源和轨道（离开房间后调用）
  // 这会销毁旧的 Source 和 Track，确保后台线程不再持有引用
...
private:
  ...
  // 使用 QPointer 而非原始指针，当 QML VideoSink 销毁时自动变为 null
  // 避免悬垂指针导致的崩溃
  ...
  static const int AUDIO_CHANNELS = 1; // 大多数麦克风是单声道
};
