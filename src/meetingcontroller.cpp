...
MeetingController::MeetingController(QObject *parent)
    : QObject(parent), m_isMicOn(false), m_isCameraOn(false),
      m_isScreenSharing(false), m_isRecording(false), m_isHandRaised(false),
      m_isInMeeting(false), m_participantCount(0), m_meetingSeconds(0),
...
void MeetingController::createMeeting(const QString &title) {
  ...
  // 生成随机会议ID（作为房间名）
  QString newMeetingId = QString::number(
      QRandomGenerator::global()->bounded(100000000, 999999999));
...
// 媒体控制实现
void MeetingController::toggleMic() { setMicOn(!m_isMicOn); }
...
void MeetingController::toggleRecording() { setRecording(!m_isRecording); }
...
void MeetingController::setupLiveKitConnections() {
  ...
  // 连接成功
  connect(m_liveKitManager, &LiveKitManager::connected, this,
          &MeetingController::onLiveKitConnected);

  // 断开连接
  connect(m_liveKitManager, &LiveKitManager::disconnected, this,
          &MeetingController::onLiveKitDisconnected);
