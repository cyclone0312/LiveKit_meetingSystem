...
VideoFrameHandler::VideoFrameHandler(QObject *parent)
    : QObject(parent)
{
}

void VideoFrameHandler::setVideoSource(std::shared_ptr<livekit::VideoSource> source)
{
...
void VideoFrameHandler::handleVideoFrame(const QVideoFrame &frame)
{
    ...
    if (!image.isNull())
    {
        ...
        livekit::LKVideoFrame lkFrame = livekit::LKVideoFrame::create(
            frameWidth, frameHeight, livekit::VideoBufferType::ARGB);
...
AudioFrameHandler::AudioFrameHandler(int sampleRate, int channels, QObject *parent)
    : QIODevice(parent), m_sampleRate(sampleRate), m_numChannels(channels)
{
    open(QIODevice::WriteOnly);
}

void AudioFrameHandler::setAudioSource(std::shared_ptr<livekit::AudioSource> source)
{
...
qint64 AudioFrameHandler::writeData(const char *data, qint64 len)
{
    ...
    try
    {
        ...
        // 发送到 LiveKit（阻塞调用）
        m_audioSource->captureFrame(audioFrame, 20); // 20ms 超时
    }
    catch (const std::exception &e)
    {
        // 静默忽略，避免日志洪泛
...
MediaCapture::MediaCapture(QObject *parent)
    : QObject(parent), m_captureSession(std::make_unique<QMediaCaptureSession>()), m_internalVideoSink(std::make_unique<QVideoSink>()), m_videoHandler(std::make_unique<VideoFrameHandler>(this))
{
    ...
    // 连接内部视频接收器的信号
    connect(m_internalVideoSink.get(), &QVideoSink::videoFrameChanged,
            this, &MediaCapture::onVideoFrameReceived);
...
void MediaCapture::refreshDevices()
{
    ...
    qDebug() << "[MediaCapture] 发现" << m_microphoneDevices.count() << "个麦克风:";
...
void MediaCapture::createLiveKitSources()
{
    ...
    m_lkVideoSource = std::make_shared<livekit::VideoSource>(VIDEO_WIDTH, VIDEO_HEIGHT);

    // 创建音频源 - 构造函数接受 (sample_rate, num_channels, queue_size_ms)
    m_lkAudioSource = std::make_shared<livekit::AudioSource>(AUDIO_SAMPLE_RATE, AUDIO_CHANNELS, 1000);

    // 设置到处理器
    m_videoHandler->setVideoSource(m_lkVideoSource);

    // 创建音频处理器
    m_audioHandler = std::make_unique<AudioFrameHandler>(AUDIO_SAMPLE_RATE, AUDIO_CHANNELS, this);
...
bool MediaCapture::isCameraActive() const
{
    return m_cameraActive;
}

bool MediaCapture::isMicrophoneActive() const
{
    return m_microphoneActive;
}

QVideoSink *MediaCapture::videoSink() const
{
    return m_externalVideoSink;
}
...
int MediaCapture::currentCameraIndex() const
{
...
{
    if (index >= 0 && index < m_cameraDevices.count() && m_currentCameraIndex != index)
    {
...
{
    if (index >= 0 && index < m_microphoneDevices.count() && m_currentMicrophoneIndex != index)
    {
...
void MediaCapture::onCameraErrorOccurred(QCamera::Error error, const QString &errorString)
{
